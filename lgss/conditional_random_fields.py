...
from lgss.model import HighDimensionalModel
from lgss.wrapper import Wrapper
from lib.math_functions import SparseMM
from lgss.modules.common import convert_region_type
  ...
  def forward(self, x):
    ...
    for i in range(self.meanfield_iterations):  # Meanfield iteration
      ...
      out = self.convs[i].apply(out, self.conv.kernel, x.pixel_dist, self.conv.stride,
                                ...
                                self.region_offset_, x.coordinate_map_key, x.coords_key, x.coordinate_manager)
      # Add unary
      out += xf
    ...
    return SparseTensor(out, coordinate_map_key=x.coordinate_map_key, coordinate_manager=x.coords_man)
